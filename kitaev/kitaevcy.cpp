...
int main(int argc, char *argv[]) {
	...
	double dt(0.02), tmax(100.), t00(0.), mui(-1.), muf;
...
char output[80];
std::sprintf(output, "data/C%dwireu%.0fk%.0fl%d.dat", n_cycle, 100*upsilon, abs(kappa), L);
...
E.hamkBuild();
E.GroundState();
std::cout << E.energy << " " << real(E.Correlations(0, 0, 'C' )) << "\n";
exit(8);
...
for(int sweep = 0; sweep < events; ++sweep) {
	...
	if( (mu-MUC) * std::pow(tau, YMU/(YMU + ZETA)) <= abs(kappa) ) {
	//if ( sweep % ( events/ int(abs(kappa)*20000) ) == 0) {
