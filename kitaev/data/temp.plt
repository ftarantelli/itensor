...
if(ARG7) {if(int(ARG7) == 1) {chr = 'k';}}

do for [N=1:5] {
dfile[N] = sprintf("wireABCu%s%s%sl%d.dat%d", ARG1, chr, ARG2, length[N], verse )
...
#if(ARG10) {set ytics real(ARG10);}

if(ARG8) {
	if(int(ARG8) == 1) {set key top right;};
	if(int(ARG8) == 2) {set key top left;};
	if(int(ARG8) == 3) {set key bottom left;};
	if(int(ARG8) == 4) {set key bottom right;};
}
...
if(var == 2) {set ylabel "L ({/Symbol r - r}_c)" offset 1., 0;}	# L^{/Arial=20 y_{/Symbol f}}
if(var == 3) {set ylabel "L C_{L/3}" offset 1.4, 0;}
if(var == 4) {set ylabel "A" offset 1.4, 0;}
...
if(ARG9) {
	if(int(ARG9) == 1) {
		...
		if(ARG7) {if(int(ARG7) == 1) {set label 1 at graph 0.75,graph 0.9 "{/Arial=30 {/Symbol Q}_{/ZapfDingbats=15 \110} = 10" tc lt 8;}};
	...
	if(int(ARG9) == 2) {
		...
		if(ARG7) {if(int(ARG7) == 1) {set label 1 at graph 0.08, graph 0.15 "{/Arial=30 {/Symbol Q}_{/ZapfDingbats=15 \110} = 10" tc lt 8;}};
	...
	if(int(ARG9) == 4) {
		...
		if(ARG7) {if(int(ARG7) == 1) {set label 1 at graph 0.75,graph 0.15 "{/Arial=30 {/Symbol Q}_{/ZapfDingbats=15 \110} = 10" tc lt 8;}};
...
#stats dfile[1] using ($1):($2*6.**expn[var-1])  #nooutput #name 'ycol'
...
if(var == 2){
set ytics 100.
plot  dfile[1] using ($1 ):($5*length[1]**expn[var-1]) t sprintf("L = %d  ", length[1]) w l lw 5. lt rgb 'dark-green' dt 4 ,\
      dfile[2] using ($1 ):($5*length[2]**expn[var-1]) t sprintf("L = %d  ", length[2]) w l lw 5. lt rgb 'cyan' dt 2,\
      dfile[3] using ($1 ):($5*length[3]**expn[var-1]) t sprintf("L = %d  ", length[3]) w l lw 5. lt rgb 'red' dt '.',\
      dfile[4] using ($1 ):($5*length[4]**expn[var-1]) t sprintf("L = %d  ", length[4]) w l lw 5. lt 1 dt 6,\
      dfile[5] using ($1 ):($5*length[5]**expn[var-1]) t sprintf("L = %d", length[5]) w l lw 5. lt 8 dt 1,\
}#      dfile[6] using ($1 ):($2*16.**expn[var-1]) t 'L = 16' w l lw 5. lt 8 dt 1,\
#      dfile[7] using ($1 ):($2*18.**expn[var-1]) t 'L = 18' w l lw 5. lt 6 dt 1,\
#}
if(var == 2){
plot  dfile[1] using ($1 ):($2*length[1]**expn[var-1]) t sprintf("L = %d  ", length[1]) w l lw 5. lt rgb 'dark-green' dt 4 ,\
      dfile[2] using ($1 ):($2*length[2]**expn[var-1]) t sprintf("L = %d  ", length[2]) w l lw 5. lt rgb 'cyan' dt 2,\
      dfile[3] using ($1 ):($2*length[3]**expn[var-1]) t sprintf("L = %d  ", length[3]) w l lw 5. lt rgb 'red' dt '.',\
      dfile[4] using ($1 ):($2*length[4]**expn[var-1]) t sprintf("L = %d  ", length[4]) w l lw 5. lt 1 dt 6,\
      dfile[5] using ($1 ):($2*length[5]**expn[var-1]) t sprintf("L = %d", length[5]) w l lw 5. lt 8 dt 1,\
      }#      dfile[7] using ($1 ):($3*18.**expn[var-1]) t 'L = 18' w l lw 5. lt 6 dt 1,\
#}
if(var == 4){
set ytics 0.2
plot  dfile[1] using ($1 ):(exp(-$4)) t sprintf("L = %d  ", length[1]) w l lw 5. lt rgb 'dark-green' dt 4 ,\
      dfile[2] using ($1 ):(exp(-$4)) t sprintf("L = %d  ", length[2]) w l lw 5. lt rgb 'cyan' dt 2,\
      dfile[3] using ($1 ):(exp(-$4)) t sprintf("L = %d  ", length[3]) w l lw 5. lt rgb 'red' dt '.',\
      dfile[4] using ($1 ):(exp(-$4)) t sprintf("L = %d  ", length[4]) w l lw 5. lt 1 dt 6,\
      dfile[5] using ($1 ):(exp(-$4)) t sprintf("L = %d", length[5]) w l lw 5. lt 8 dt 1,\
}#      dfile[6] using ($1 ):(exp(-$4)) t 'L = 16' w l lw 5. lt 8 dt 1,\
#      dfile[7] using ($1 ):($4*18.**expn[var-1]) t 'L = 18' w l lw 5. lt 6 dt 1,\
#}

# "~{abc}{.5 \\_} "
# L200 = gamma=0-C360     L300 = gamma=0-P360
#   "\\U+0286 \\U+13DD \\U+2112"   Ꮭ   ℒ   ʆ
...
set label 1 at screen 0.245, screen 0.855 "{/Symbol=28 W} = 1"

f(x)=a*x + b 
#fit f(x) "estrUp1hi1_8.dat" u (1./$1):2 via a,b


#plot  f(x) notitle w l dashtype '_' lt 1, "estrUp1hi1_8.dat" using (1./$1):2 t '' pt 7 ps 2 # lt 8
