...
inline void wire::derivate( double t, const cx_dvec& _gsk, cx_dvec& kvec, int k) {
		...
		for(int j=0; j<int(L/2); ++j)
			hamk(j) =  hamk0(j) + (mu + signn * t / tau) * vpk(j);
		//K.hamk(j) = K.hamk(j) + signn * dt * K.vpk(j);
		//mu += signn * dt / tau;
...
void wire::RungeKuttaPsi(double _dt) {
	...
	for(int k=0; k<int(L/2); ++k) {
//std::cout << "aa\n";
		derivate( 0.         , gs(k)  , kvec1, k);
//	std::cout << "aa1\n";
		temp = gs(k) + kvec1*_dt/2.;
		derivate( _dt/2., temp, kvec2, k);
		temp = gs(k) + kvec2*_dt/2.;
		derivate(  _dt/2., temp, kvec3, k);
		temp = gs(k) + kvec3*_dt;
		derivate( _dt   , temp, kvec4, k);
...
for(int kitem=0; kitem <= k_events; ++kitem) {
	if ( kitem%(k_events/100) == 0) {
		std::cout << int(kitem/float(k_events)*100.) << "%\n";
		//out_file << std::flush;
	}
	...
	K.time = (mu - MUC)*tau; E.time = K.time;
	K.GroundState();
	...
	for(int sweep = 0; sweep < events; ++sweep) {
		...
		K.time += dt;
		mu += signn * dt / tau;
		//if( sweep == events/2 - 1 ) signn = -signn;
		if( (sweep+1) % (events/2/n_cycle) == 0 ) signn = -signn;
		//if( mu + dt/tau > muf ) {
